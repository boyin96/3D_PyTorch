...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = torch.arange(B, dtype=torch.int64).to(device).view(view_shape).repeat(repeat_shape)
...
def farthest_point_sample(xyz, npoint):
    """
        For same input points, N
    Args:
        xyz: point cloud data, [B, N, 3] (B->batch size, N->total number of sample points, 3->xyz coordinates)
    ...
    device = xyz.device
    B, N, _ = xyz.shape
    centroids = torch.zeros(B, npoint, dtype=torch.int64).to(device)
    distance = torch.ones(B, N).to(device) * 1e10
    farthest = torch.randint(0, N, (B,), dtype=torch.int64).to(device)
    batch_indices = torch.arange(B, dtype=torch.int64).to(device)
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    B, N, _ = xyz.shape
    _, S, _ = new_xyz.shape
    group_idx = torch.arange(N, dtype=torch.int64).to(device).view(1, 1, N).repeat([B, S, 1])
...
class PointNetClsSSG(nn.Module):
    def __init__(self, num_class, normal_channel=True):
        super(PointNetClsSSG, self).__init__()

        self.normal_channel = normal_channel
        in_channel = 6 if self.normal_channel else 3
...
class PointNetClsMSG(nn.Module):
    def __init__(self, num_class, normal_channel=True):
